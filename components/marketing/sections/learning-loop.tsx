    ...
    <section className="bg-white px-6 py-28 sm:py-36">
      <div className="mx-auto max-w-[1320px]">
        <Reveal className="mx-auto max-w-2xl text-center">
          <p className="text-balance text-base font-bold uppercase tracking-[0.16em] text-brand sm:text-lg">Your Training Loop</p>
          ...
          {FRAMES.map((frame, i) => (
            <Reveal key={frame.eyebrow} delay={i * 100} className="group flex flex-col gap-4">
              ...
              <div>
                ...
                <p className="text-pretty mt-1.5 text-base text-[#414B57]">{frame.copy}</p>
              </div>
            </Reveal>
          ))}
        </div>
        ...
        <Reveal delay={400} className="mx-auto mt-16 max-w-4xl border-t border-[#E5E8EC] pt-10 text-center">
          ...
          <p className="text-pretty mt-3 text-base text-[#414B57]">
          ...
          <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-3">
            {FORMATS.map(({ icon: Icon, label, copy }) => (
              <div key={label} className="flex flex-col items-center gap-2 text-center">
                ...
                <p className="text-pretty text-base text-[#414B57]">{copy}</p>
              </div>
            ))}
          </div>
        </Reveal>
