...
import { isDevelopment, isProduction, isStaging, v2StagingUsesRealData } from "@/lib/env";
 ...
 * below: the intent (a real staging student must never wander from Home's
 * real data into fixture content) stands, but it only applies once
 * v2StagingUsesRealData() is true. Until then staging must reproduce the
 * complete approved V2 fixture reference experience exactly like
 * development -- PrototypeChrome, full navigation, Mia/Jake -- so v2FixtureMode
 * below is the one place both "modes" are decided, matching lib/env.ts's
 * v2StagingUsesRealData() doc comment. The STAGING badge is unrelated to
 * this and still always shows in staging, real-data mode or not.
 */
export default async function V2Layout({ children }: { children: ReactNode }) {
  ...
  const v2FixtureMode = isDevelopment() || (isStaging() && !v2StagingUsesRealData());
    ...
    <div className="min-h-dvh bg-surface-sunken">
      ...
      <div className="mx-auto min-h-dvh max-w-lg bg-surface-sunken pb-24">
        ...
        <Suspense fallback={null}>
          <AppHeader
            homeHref="/v2"
            actions={<V2HeaderActions startFlightDisabled={!v2FixtureMode} profileNavDisabled={!v2FixtureMode} />}
      ...
      <BottomNav
        ...
        disabledKeys={v2FixtureMode ? [] : ["train", "debrief", "progress"]}
      />
    </div>
  );
}
