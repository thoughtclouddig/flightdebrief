...
export default async function DebriefResultsPage(props: PageProps<"/flights/[id]/debrief/results">) {
  ...
  const isDemo = viewer.organization.demoExpiresAt != null;
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-8">
      <div>
        ...
        {/* Instructor/admin only, and only worth showing when it's NOT Claude.
            A missing or rate-limited ANTHROPIC_API_KEY makes lib/ai/index.ts
            fall back to the local heuristic analyzer silently -- the debrief
            still completes and looks normal, so a thin result otherwise looks
            like the analysis did badly rather than like it never ran.

            Never in the demo. The CFI persona IS an instructor, so this was
            showing our own API-key troubleshooting to every prospect who
            opened a debrief -- naming an environment variable to someone
            evaluating the product. */}
        {isInstructorViewer && !isDemo && debrief.analyzedWith === "mock" ? (
          <p className="mt-2 rounded-lg bg-surface-sunken px-3 py-2 text-xs text-amber-ink">
            Generated by the offline fallback analyzer, not AI -- results will be thinner than usual. Check that
