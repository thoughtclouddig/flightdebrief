...
- sections: 4 to 6 of them. Each has:
    ...
    steps: Use these wherever the section genuinely describes something done in order -- what to do before a lesson, how to run a manoeuvre, how to come back from a gap. Most articles about training have at least one such section, and a procedure written as a paragraph beginning "First," is harder to follow than the same thing numbered. Only when order matters, and only with 2 or more steps. A numbered list of unordered points is a paragraph wearing a costume. Most sections have none. Each step is one short imperative sentence.
    ...
    checklist: OPTIONAL. Things to confirm or check, in any order -- distinct from steps, which are sequential. 2 or more.
    comparison: OPTIONAL. When the section turns on a distinction the reader keeps getting wrong (rehearsal vs review, currency vs proficiency), give both sides: leftLabel/left and rightLabel/right, one or two sentences each. Only when both sides are genuinely in the copy.
