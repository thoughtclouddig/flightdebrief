...
/**
 * Add Flight -- the complete fixture-backed 5-stage demo, shared between
 * app/prototype/vector/flights/new/page.tsx and app/v2/flights/new/page.tsx.
 * `myFlightsHref`/`debriefNewHref`/`homeHref` are the only seams.
 *
...
export function AddFlightDemo({
  ...
  return (
    <Screen>
      {stage === "saved" ? null : <BackLink href={myFlightsHref}>My flights</BackLink>}
      ...
      {stage === "saved" ? (
        <Saved picked={picked} solo={solo} myFlightsHref={myFlightsHref} debriefNewHref={debriefNewHref} homeHref={homeHref} />
      ) : null}
    </Screen>
  );
}
  ...
  return (
    <>
      <Panel>
        ...
        <PanelHeadline>{picked ? `${picked.departureAirport} → ${picked.arrivalAirport}` : "Today's flight"}</PanelHeadline>
      ...
      <div className="flex flex-col gap-2.5">
        <PrimaryButton href={debriefNewHref}>
          <Mic className="size-[18px]" aria-hidden />
          {solo ? "Add my reflection" : "Start debrief"}
        </PrimaryButton>
        <div className="flex gap-2.5">
          <SecondaryButton href={myFlightsHref}>My flights</SecondaryButton>
          <SecondaryButton href={homeHref}>Do it later</SecondaryButton>
        </div>
      </div>
    </>
  );
