    ...
    <section className="bg-white px-6 pb-20 pt-32 sm:pb-28 sm:pt-36">
      <div className="mx-auto max-w-3xl">
        <Reveal>
          ...
          <p className="mt-3 text-sm text-[#414B57]/70">Last updated: {updated}</p>
        </Reveal>
...
export function LegalSection({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div>
       ...
       */}
      <div className="mt-3 flex flex-col gap-3 leading-relaxed text-[#414B57] [&_p]:text-pretty">{children}</div>
    </div>
  );
}
