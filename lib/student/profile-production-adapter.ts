 ...
 *
 * instructorHref is expected to be a self-link (each caller's own /profile
 * or /v2/profile) rather than a real instructor-detail page -- there isn't
 * one, and the approved fixture reference (app/v2/profile/page.tsx) already
 * made this exact choice for the identical reason, so this follows it rather
 * than inventing a different answer for the same non-existent destination.
 *
...
export async function buildProductionProfileProps(
  repo: Repository,
  viewer: Viewer,
  hrefs: { flightsHref: string; debriefsHref: string; instructorHref: string; guideHref: string; supportHref: string },
  ...
  return {
    ...
    flightsHref: hrefs.flightsHref,
    flightsCount: flights.length,
    debriefsHref: hrefs.debriefsHref,
    debriefsCount: String(debriefedCount),
    instructorHref: hrefs.instructorHref,
    instructorName: instructors.length > 0 ? instructors.map((i) => i!.name).join(", ") : "None yet",
    guideHref: hrefs.guideHref,
    supportHref: hrefs.supportHref,
